...
import effects
import refdata
import spell_view as spell_view_module
...
def build_character_view(char, db):
    ...
    # Spell-siden (forberedte/kendte slots, ⚡ Kast-knap, kategori-E/F "I brug",
    # summon-/cure-kataloger, domæne-spells, gnome-SLA'er) er udtrukket til sit
    # eget view-modul — samme mønster som companion_module/wild_shape_module.
    sv = spell_view_module.build_spell_view(char, db)
    ...
    def _atk_fields(atk):
        ...
        return {**e,
                ...
                "base_dmg": b["damage"],
                # ekstra skade-terninger — vises som separat rul-badge, ikke smeltet
                # ind (dice.py kan kun ét terning-led). To kilder: våbnets egne magiske
                # abilities (flaming → +1d6 ild, bæres på atk.bonus_dice) + aktive buffs
                # (Flame Arrow). Våben-riderne først, så de står nærmest angrebet.
                "bonus_dice": list(atk.bonus_dice) + effects.damage_dice(active_modifiers, atk.kind)}
    ...
    for d in char_module.derive_spell_attacks(char, db):
        ...
        attack_rows.append({
            ...
            **_atk_fields(atk),
            ...
            "alt_note": d["alt_note"],
            "mode": d["mode"],
            "shots": d["shots"], "auto_hit": d["auto_hit"],
        })
